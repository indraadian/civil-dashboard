@props([
    'reportEndpointUrl' => '/imports/{id}/report',
    'eventName' => 'open-import-report-modal',
])
...
<x-ui.modal x-data="{
    ...
    fetchReport(id) {
        ...
        this.loading = true;
        this.report = null;
        const endpoint = '{{ $reportEndpointUrl }}'.replace('{id}', id);
        fetch(endpoint)
            .then(res => res.json())
            .then(data => {
                this.report = data;
           ...
           <div x-on:{{ $eventName }}.window="fetchReport($event.detail.id)" class="no-scrollbar relative w-full max-w-[650px] overflow-y-auto rounded-3xl bg-white p-6 dark:bg-gray-900 lg:p-9">
        ...
        <template x-if="!loading && report">
            <div class="mt-6 space-y-6">
                {{-- Stats Grid --}}
                <div cla
          s             s="grid grid-cols-2 gap-3 sm:grid-cols-4">
      ...
      v                 >
                    <div class="rounded-2xl border border-green-100 bg-green-50/50 p-4 text-center dark:border-green-500/20 dark:bg-green-500/10">
                        <span class="text-xs text-green-600 dark:text-green-400">Berhasil</s
 p                          an>
  ...
  v                     >
                ...
                {{-- Time details --}}
                <div class="flex flex-wrap items-center justify-between text-xs text-gray-500 da
                           rk:text-gray-400 px-1">
                    <div>Waktu Mulai: <span class="font-medium text-gray-700 dark:text-gray-300" x
   -                        text="report.started_at || '-'"></span></div>
                    <div>Waktu Selesai: <span class="font-medium text-gray-700 dark:text-gray-300" x-text="report.finished_at || '-'"></span></div>
                </div>

                {{-- Error summary if failed --}}
                <templat
              e          x-if="report.error_message">
