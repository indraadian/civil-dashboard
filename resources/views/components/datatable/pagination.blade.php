...
<div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
    ...
    <div class="flex items-center gap-2">
        ...
        <select @change="changePerPage($event.target.value)"
            class="h-9 rounded-lg border border-gray-300 bg-transparent px-2.5 text-sm text-gray-700 shadow-theme-xs focus:border-blue-300 focus:outline-none focus:ring-2 focus:ring-blue-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:focus:border-blue-800">
            <template x-for="opt in config.perPageOptions" :key="opt">
                <option :value="opt" :selected="opt === perPage" x-text="opt"></option>
            </template>
        </select>
